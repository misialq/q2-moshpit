...
def run_fastp(input_sequences: SingleLanePerSampleSingleEndFastqDirFmt, trim_front1: int = 0, trim_tail1: int = 0, cut_window_size: int = 4, cut_mean_quality: int = 20, n_base_limit: int = 5, length_required: int = 15, qualified_quality_phred: int = 15, unqualified_percent_limit: float = 40.0, compression: int = 2, thread: int = 3) -> SingleLanePerSampleSingleEndFastqDirFmt:
    output_sequences = SingleLanePerSampleSingleEndFastqDirFmt()
    for sample_id, filepaths in input_sequences.sequences.iter_views(FastqGzFormat):
